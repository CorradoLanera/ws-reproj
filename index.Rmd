...
# All this code together is really really HORRIBLE and confusing!
...
- (`{here}`)<sup>*</sup> (your .orange[path will work] on every computer!)

- git + GitHub (you can .orange[collaborate] and .orange[share] your code, tracking your and others' changes at every single lines levels)

- `{renv}` (packages .orange[updates won't scare] you anymore!)

- `{devtools}` + DESCRIPTION + R/ + tests/ (you know exactly where your functions are, with .orange[provable confidence] of their behavior... you have to press nothing more than `CTRL` + `SHIFT` + `T`)

- (`analysis.R`)<sup>*</sup> + `report.R` (directly inject your - __Updated__ - objects into your reports and sample code)

- private data-raw/, (data/)<sup>*</sup>, and output/ (.orange[everyone can read] your code, only your collaborators and .orange[authorized people will be able to run it]! Still, __you can share__ a selection of derived object to anyone!)
...
- powered by `{targets}`:
  ...
  + one-click update of all the outdated dependencies for updates (and only those)
...
- created objects are directly usable on computations, reports, and tests (maintaining their privacy if shared storage system is implemented)
...
---
# .orange[Bonus-unexplored] features
...
- **Bugs will be there**. For sure! (The greatest error you can do is to assume your code is going to be error-free)
